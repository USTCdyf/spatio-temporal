...
class LearnableFourierPositionalEncoding(nn.Module):   ##这个没用上，不用管
...
class AdativeFourierNeuralOperator(nn.Module):    ##从别人拿来的 不用改动
...
class Fourier_Model(nn.Module):
    def __init__(self, shape_in, hid_S=16, hid_T=256, N_S=4, N_T=8, incep_ker=[3,5,7,11], groups=8):
        ...
        self.swin_transformer = SwinSubBlock(dim=T*hid_S, 
        ...
        self.vitfour = AFourierTransformer(img_size=128,
                                       patch_size=16,
                                       in_channels=1,
                                       out_channels=1,
                                       embed_dim=64,
    ...
    def forward(self, x_raw):
        ...
        global_feature = self.vitfour(x_raw)
        ...
        return Y + global_feature




if __name__ == '__main__':
    x = torch.randn((16, 10, 1, 128, 128))
    y = torch.randn((16, 10, 1, 128, 128))
    model1 = Fourier_Model(shape_in=(10,1,128,128))
    output = model1(x)
