...
class SwinDecoder(nn.Module):
    def __init__(self, C_hid, C_out=1):
        super(SwinDecoder, self).__init__()

        layer_list = []

        for i in range(2): #这里5是因为经过标准swin transformer后,图像大小会缩小32倍, 2**5
            layer_list.append(ConvSC(C_hid, C_hid//2, stride=2, transpose=True))
            C_hid = C_hid //2
        self.dec = nn.Sequential(*layer_list)
...
if __name__ == '__main__':
    # torch.Size([160, 1, 128, 128]) torch.Size([160, 16, 32, 32]) torch.Size([160, 16, 128, 128])

    model = SwinTransformer(img_size=128, in_chans=1, embed_dim=32,
                            depths=(2, 2, 6, 2), num_heads=(3, 6, 12, 24))
    decoder = SwinDecoder(256, 1)
    x = torch.randn((160, 1, 128, 128))
    y = model(x)
    y = y.permute([0,3,1,2])
    print(y.shape)
    ...
    config.depths = [2, 2, 3, 3]
    model = SwinModel(config=config, add_pooling_layer=False)
    inputs = {'pixel_values': x}
    outputs = model(**inputs, output_hidden_states=True)
    print(outputs.reshaped_hidden_states[-1].shape)
